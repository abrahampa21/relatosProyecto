...
import { RavenIcon, GlowingCatEyesIcon, ApeIcon, DevilIcon, FeatherIcon, BloodyHandprintIcon, BlackCatIcon } from './icons';
...
const StoryAmbiance: React.FC<StoryAmbianceProps> = ({ animationType, scrollProgress }) => {
  ...
  const RavenAmbiance = () => (
    <>
      ...
      <div 
        ...
        style={{ transform: `translateX(${scrollProgress > 20 ? '120vw' : '0'}) rotate(15deg)`, opacity: 0.8 }}
      >
        <RavenIcon />
      </div>
       <div 
        className="fixed top-[60%] -right-24 w-24 h-24 text-gray-900 transition-transform duration-[5000ms] ease-out"
        style={{ transform: `translateX(${scrollProgress > 60 ? '-120vw' : '0'}) rotate(-25deg)`}}
      >
        <RavenIcon />
      </div>
      ...
      {[...Array(5)].map((_, i) => (
        <div 
            ...
            style={{ 
                top: '-5%', 
                left: `${10 + i * 20}%`,
                transform: `translateY(${scrollProgress > (15 * (i+1)) ? '105vh' : '0'}) rotate(${i * 30}deg)`,
  ...
  const BlackCatAmbiance = () => (
    <>
        ...
        <div 
            ...
            style={{ opacity: scrollProgress > 30 && scrollProgress < 50 ? 0.7 : 0 }}
        >
            <GlowingCatEyesIcon />
        </div>
        <div 
            className="fixed bottom-1/4 left-8 w-24 h-24 transition-opacity duration-1000"
            style={{ opacity: scrollProgress > 70 && scrollProgress < 90 ? 0.7 : 0 }}
        >
            <GlowingCatEyesIcon />
        </div>
        {/* Scratch marks */}
        <div 
            ...
            style={{ 
                ...
                transform: `scale(${scrollProgress > 55 && scrollProgress < 58 ? 1.2 : 1}) rotate(-15deg)`
            }}
        >
        ...
        <div 
            ...
            style={{
                transform: `translateX(${scrollProgress > 85 && scrollProgress < 90 ? '120vw' : '0'})`
            }}
        >
            <BlackCatIcon />
        </div>
  ...
  const MorgueAmbiance = () => (
    <>
        ...
        <div 
            ...
            style={{ opacity: scrollProgress > 60 && scrollProgress < 75 ? 0.15 : 0 }}
        >
        ...
        <div
            ...
            style={{ opacity: scrollProgress > 25 && scrollProgress < 35 ? 0.6 : 0 }}
        >
            <BloodyHandprintIcon/>
        </div>
        ...
        <div 
            ...
            style={{ 
                background: 'radial-gradient(circle at 20% 30%, rgba(139, 0, 0, 0.3) 0%, transparent 20%)',
        ...
        <div 
            className="fixed inset-0 pointer-events-none transition-opacity duration-500"
            style={{ 
                background: 'radial-gradient(circle at 80% 70%, rgba(139, 0, 0, 0.2) 0%, transparent 15%)',
  ...
  const BelfryAmbiance = () => (
     <>
        <div className="fixed top-0 left-1/2 -translate-x-1/2 h-48 w-2 bg-gray-700" style={{ transformOrigin: 'top center', animation: scrollProgress > 10 ? `swing 4s ease-in-out infinite` : 'none' }}>
            <style>
                {`
                @keyframes swing {
            ...
            {[...Array(13)].map((_, i) => (
                <div 
                    ...
                    style={{
                        ...
                        transform: `
                            rotate(${scrollProgress > 65 && scrollProgress < 75 ? (i * 27.6 - 90) * 3 : i * 27.6 - 90}deg) 
                            translateY(${scrollProgress > 65 && scrollProgress < 75 ? '40vh' : '0'})
                            scale(${scrollProgress > 65 && scrollProgress < 75 ? 2 : 0})
                        `
                    }}
                >
                    {i + 1}
  ...
  return <div className="pointer-events-none z-0 overflow-hidden fixed inset-0">{renderAmbiance()}</div>;
};
